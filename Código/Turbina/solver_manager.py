...
import pickle
import os
from Turbina.axial_turb_solver import *
#from axial_turb_solver import *
from Turbina.multi_sweep_plotter import function_for_itering_csvfiles
...
def data_to_df(process_object: solver_object, req_vars=None) -> [pd.DataFrame | None, pd.DataFrame | None,
    ...
    contenido almacenado, en caso de que no se especifique se establece la configuración por defecto.
...
def main_turbina(variables_comp, variables_cc, resultados_cam_comb, archivo):
    data_dictionary = txt_reader(archivo)
    settings = None
    try:
    ...
    def operations_for_solver_init():
        ...
        gas_model = gas_model_to_solver(thermod_mode=data_dictionary.get('thermod_mode_in_gas_model_module', 'ig'), C_atoms=int(variables_cc['x']), H_atoms=int(variables_cc['y']), air_excess=float(resultados_cam_comb['n']))
    ...
    if mode == 'process_nominal_values':
        ...
        try:
            ...
            if data_dictionary['chain_mode']:
                ...
                resultados_turbina = solver.problem_solver(
                    T_in= resultados_cam_comb['T_inlet'],
                    p_in= resultados_cam_comb['p_inlet'],
                    n_rpm= float(variables_comp['w_rpm']),
                    p_out= None, #data_dictionary['p_outlet'],
                    m_dot= resultados_cam_comb['mass_flow'],
                    C_inx=None, #data_dictionary['axial_inlet_velocity'],
                    C_inx_ref= None, #resultados['c_inx'], #data_dictionary['reference_inlet_velocity'],
                )
                return resultados_turbina
                #solver_data_saver(solver_file_name, solver)
        except NameError:
            raise InputDataError('Non-valid text file, please, stick to the template.')
...
if __name__ == '__main__':
    ...
    variables = {}
    resultados_cam_comb = {}
    print(main_turbina(variables, resultados_cam_comb))
