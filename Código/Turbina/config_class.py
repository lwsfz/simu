...
from Turbina.gas_modeling import mixpm
...
class config_class:
    ...
    def set_geometry(self, B_A_est: int | float | list, B_A_rot: int | float | list, cuerda: int | float | list,
        ...
        for num, h in enumerate(geom['H']):   # h: 0 1 2 3 4 ... Rm: 0 0 1 2 3
            num2 = num - 1 if num > 0 else 0
            if (2*Rm[num2]+h)/(2*Rm[num2]-h) >1.45:
...
class gas_model_to_solver:
    ...
    C_atoms: float | int = 10.0  # Átomos de carbono en cada átomo de hidrocarburo en los reactivos.
    H_atoms: float | int = 22.0  # Átomos de hidrógeno en cada átomo de hidrocarburo en los reactivos.
    air_excess: float | int = 2.56146814  # Exceso de aire considerado en el ajuste estequiométrico de la combustión completa.
